#if (UNITY_ANDROID && !(UNITY_EDITOR)) && !GRAVITY_BYTEDANCE_GAME_MODE && !GRAVITY_BYTEDANCE_TT_GAME_MODE && !GRAVITY_KUAISHOU_GAME_MODE && !GRAVITY_KUAISHOU_WEBGL_GAME_MODE && !GRAVITY_OPPO_GAME_MODE && !GRAVITY_HUAWEI_GAME_MODE && !GRAVITY_XIAOMI_GAME_MODE && !GRAVITY_ALIPAY_GAME_MODE
...
namespace GravityEngine.Wrapper
{
    public partial class GravityEngineWrapper
    {
        ...
        private static void init(GravityEngineAPI.Token token)
        {
            ...
            configDic["enableIMEI"] = token.enableIMEI;
            configDic["enableOAID"] = token.enableOAID;
            configDic["enableAndroidId"] = token.enableAndroidId;
            configDic["enableMAC"] = token.enableMAC;
        ...
        private static string track(string eventName, string properties, DateTime dateTime)
        {
            AndroidJavaObject date = getDate(dateTime);
            AndroidJavaObject tz = null;
            return getInstance().Call<string>("track", eventName, getJSONObject(properties), date, tz);
        }

        private static string track(string eventName, string properties, DateTime dateTime, TimeZoneInfo timeZone)
        {
            AndroidJavaObject date = getDate(dateTime);
            AndroidJavaObject tz = null;
        ...
        private static string track(string eventName, string properties)
        {
            return getInstance().Call<string>("track", eventName, getJSONObject(properties));
        }
        ...
        private static void queryDryRunInfo(string traceId, Dictionary<string, object> otherParams, IQueryDryRunCallback queryDryRunCallback)
        {
            GravityHelper.QueryDryRunInfo(traceId, otherParams, queryDryRunCallback);
        }
        
        private static void filterAndReportPayEventToTencent(int payAmount, string traceId)
        {
            GE_Log.d("android not support filterAndReportPayEventToTencent");
        }

        private static void initializeIOS(bool enableAsa, string clientId, string caid1_md5, string caid2_md5, bool enableSyncAttribution, string channel, IInitializeCallback initializeCallback)
        {
        ...
        private static string trackPayEvent(int payAmount, string payType, string orderId, string payReason,
            string payMethod)
        {
            return getInstance().Call<string>("trackPayEvent", payAmount, payType, orderId, payReason, payMethod);
        }
